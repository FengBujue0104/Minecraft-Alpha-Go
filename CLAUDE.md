...
2. The goroutine runs **`generateTerrain` only**, then sends the chunk on `genResults` (buffered 16).
...
Boundary faces depend on whether the adjacent chunk exists, which is why both `insertChunk` and `UnloadChunk` mark neighbours dirty. `UnloadChunk` deletes from the map *before* marking, so the rebuild sees the chunk as gone. `FlushGenerations` drains generations and then the entire dirty queue, so the player never spawns into an unrendered world.
...
Measured frame times, same scene and seed, 300 frames, target FPS off, GTX 750 Ti:

| | no cull | with cull |
|---|---|---|
| loadDist 1 | 17.3 ms | 16.5 ms (~61 fps) |
| loadDist 2 | 36.4 ms | 27.3 ms (~37 fps) |
| loadDist 3 | 58.9 ms | 41.8 ms (~24 fps) |

Raising `loadDist` past 1 needs a real per-chunk `rl.Mesh` built once on visibility change — that is the only way past the per-draw-call FFI cost. The dirty queue is already the "geometry changed" signal such a rewrite would hang off.
