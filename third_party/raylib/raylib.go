...
// NOTE(fork): upstream's init() called runtime.LockOSThread() here ("bind
// the main goroutine to the main thread" for desktop GLFW). Under Go 1.26
// with -buildmode=c-shared on Android, package init runs inside dlopen on
// the NativeActivity UI thread, and locking it aborts the process before
// any Go code runs — every raylib app crashed instantly at launch (see
// android/canary: A/C pass, D/E die with zero visuals). Locking is only
// meaningful for the desktop cgo windowing path, which this project does
// not use (desktop builds go through purego), so the init is removed.
